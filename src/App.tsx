      ...
      <main id="top">
        ...
        <section id="about" className="about section-grid section-space">
          ...
          <div className="about-content" data-reveal data-reveal-delay="1">
            ...
            <a className="text-link" href="https://www.linkedin.com/in/arya-26-shah/" target="_blank" rel="noreferrer" style={{ marginTop: '28px' }}>More on LinkedIn <Linkedin size={16} /></a>
          </div>
        </section>

        <section id="trajectory" className="trajectory section-space">
          <div className="section-kicker" data-reveal>02 <span>Career trajectory</span></div>
          <div className="trajectory-layout">
            <div data-reveal data-reveal-delay="1"><h2>Growing through<br /><em>real responsibility.</em></h2><p className="trajectory-intro">My path has moved from building and deploying practical systems to owning enterprise-grade financial workflows. Each role has sharpened how I think about reliability, collaboration, and the details that make software dependable.</p></div>
            <div className="timeline">
              <div className="timeline-item" data-reveal><div className="timeline-date">May 2026 \u2014 Present</div><div className="timeline-marker"><Briefcase size={16} /></div><div><p className="timeline-company">MRI Software - Vadodara, India</p><h3>Software Development Engineer 1</h3><p>Building enterprise financial and workflow automation systems using C#, .NET Web API, SQL Server, and React. Optimizing SQL queries and backend workflows to improve performance, building REST APIs for production-ready applications, and contributing through code reviews, deployments, and Agile delivery.</p></div></div>
              <div className="timeline-item" data-reveal><div className="timeline-date">Jan 2026 \u2014 May 2026</div><div className="timeline-marker"><Briefcase size={16} /></div><div><p className="timeline-company">MRI Software - Vadodara, India</p><h3>Software Engineering Intern</h3><p>Built CRM workflow automation modules with React, JavaScript, .NET, and PostgreSQL. Improved reusable frontend components, SQL migration scripts, backend optimizations, debugging, deployments, and production support.</p></div></div>
              <div className="timeline-item" data-reveal><div className="timeline-date">Oct 2024 \u2014 Apr 2025</div><div className="timeline-marker"><Briefcase size={16} /></div><div><p className="timeline-company">Shakti Infotech - Vadodara, India</p><h3>Software Development & DevOps Intern</h3><p>Contributed to enterprise cheque printing systems using ASP.NET MVC, SQL Server, and Azure DevOps. Managed CI/CD workflows, deployments, debugging, database operations, backend modules, SQL procedures, and production support activities.</p></div></div>
            </div>
          </div>
        </section>

        <section id="work" className="work section-space">
          <div className="section-heading" data-reveal><div><div className="section-kicker">03 <span>Selected work</span></div><h2>Things I've made<br /><em>and learned from.</em></h2></div><p className="heading-aside">A selection of projects across enterprise software, digital products, and applied intelligence.</p></div>
          <div className="project-grid">{projects.map((project, index) => <article className={`project-card ${project.featured ? 'featured' : ''}`} key={project.number} data-reveal data-reveal-delay={index % 2 ? '2' : '1'}><div className="project-top"><span className="project-number">{project.number}</span><ArrowUpRight className="project-arrow" size={20} /></div><p className="project-type">{project.type}</p><h3>{project.title}</h3><p className="project-description">{project.description}</p><div className="tag-list">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><div className="project-links"><a href={project.href} target={project.href.startsWith('http') ? '_blank' : undefined} rel={project.href.startsWith('http') ? 'noreferrer' : undefined}>{project.href.startsWith('http') ? 'View live project' : 'Discuss this project'} <ArrowUpRight size={14} /></a>{project.github && <a href={project.github} target="_blank" rel="noreferrer">GitHub <Github size={14} /></a>}</div></article>)}</div>
        </section>

        <section id="capabilities" className="capabilities section-space">
          <div className="section-kicker" data-reveal>04 <span>Capabilities</span></div>
          <div className="capabilities-layout" data-reveal data-reveal-delay="1"><div><h2>From first thought<br />to <em>finished system.</em></h2><p className="capability-intro">I bring a builders' mindset to every stage asking better questions early, sweating the details, and keeping the end user in view.</p></div><div className="capability-list"><div className="capability-item"><span>01</span><div><Code2 size={22} /><h3>Product-minded engineering</h3><p>Interfaces and APIs that feel considered, not merely complete.</p></div></div><div className="capability-item"><span>02</span><div><Database size={22} /><h3>Systems & data</h3><p>Structured thinking for workflows, databases, and operational reliability.</p></div></div><div className="capability-item"><span>03</span><div><Sparkles size={22} /><h3>Learning in public</h3><p>Curiosity, documentation, and collaboration as part of the craft.</p></div></div></div></div>
          <div className="skills-grid" data-reveal>{skills.map((skill) => <div className="skill-row" key={skill.label}><strong>{skill.label}</strong><span>{skill.value}</span></div>)}</div>
        </section>

        <section id="contact" className="contact section-space"><div className="contact-card" data-reveal><div className="contact-copy"><div className="section-kicker light">05 <span>Start a conversation</span></div><h2>Have a problem<br /><em>worth solving?</em></h2><p>Tell me a little about what you're building, exploring, or figuring out. I'd love to hear the thinking behind it.</p><div className="contact-links"><a href="mailto:aryapshah2005@gmail.com"><Mail size={17} /> aryapshah2005@gmail.com</a><a href="https://www.linkedin.com/in/arya-26-shah/" target="_blank" rel="noreferrer"><Linkedin size={17} /> LinkedIn profile</a></div></div><form className="contact-form" onSubmit={handleSubmit}><label>Your name<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Jane Smith" /></label><label>Email address<input required type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="jane@company.com" /></label><label>What\u2019s on your mind?<textarea required rows={4} value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} placeholder="Tell me a little about your idea..." /></label><button className="button button-light" type="submit" disabled={status === 'sending'}>{status === 'sending' ? 'Sending\u2026' : status === 'sent' ? <>Message sent <Check size={16} /></> : <>Send message <Send size={15} /> </>}</button>{status === 'error' && <p className="form-message error">Something went wrong. Please email me directly.</p>}{status === 'sent' && <p className="form-message success">Thanks \u2014 your message is on its way.</p>}</form></div></section>
      </main>
